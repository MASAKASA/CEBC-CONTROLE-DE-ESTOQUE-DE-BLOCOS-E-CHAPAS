...
Function cadastrarEEditar(estoque As objEstoque)
    ...
    If cadastro = True Then ' Se cadastro
        ...
        strSql = "INSERT INTO Estoques ( nome_empresa, ativo )VALUES ('" & estoque.nome & "', " & estoque.ativo & ");"
        
        rs.Open strSql, CONEXAO_BD, adOpenKeyset, adLockPessimistic
    Else
        ' Se edição
        strSql = "UPDATE Estoques SET nome_empresa = '" & estoque.nome & "', ativo = '" & estoque.ativo _
            & "' WHERE Id_Estoque = " & estoque.id & ";"
...
Function excluir(id As String)
    ...
    'Faz a consulta para saber se o código do bloco já exite
    strSql = "UPDATE Estoques SET ativo = '" & estoque.ativo _
            & "' WHERE Id_Estoque = " & id & ";"
...
Function pesquisarPorId(id As String) As objEstoque
    ...
    While Not rs.EOF
        ...
        estoque.ativo = rs.Fields("ativo").Value
...
' Pesquisa objeto
Function listarEstoque() As Collection
    ' String para consultas
    Dim strSql As String ' String para consultas
    Dim rs As ADODB.Recordset ' Recordset para consulta principal
    
    ' String para consulta
    strSql = "SELECT * FROM Estoques ORDER BY nome_empresa;"
    ...
    ' Retorna pesquisa
    Set listarEstoque = listaEstoques
